...
class Admin(User):
    ...
    def search_chats(self, q, active, page):
        matching_chats = None
        if q != "" and active == "any":
            matching_chats = Chat.query.join(RegularUser).filter(db.or_(RegularUser.username.ilike(f'%{q}%'), Chat.id.ilike(f'%{q}%')))
        elif q != "":
            is_active = True if active == "true" else False
            matching_chats = Chat.query.join(RegularUser).filter(db.or_(RegularUser.username.ilike(f'%{q}%'), Chat.text.ilike(f'%{q}%')), Chat.active == is_active)
        elif active != "any":
            is_active = True if active == "true" else False
    ...
    def search_users(self, q, active, page):
        ...
        if q != "" and active == "any":
            ...
            is_active = True if active == "true" else False
            matching_users = User.query.join(RegularUser).filter(db.or_(RegularUser.username.ilike(f'%{q}%'), User.email.ilike(f'%{q}%'), User.id.ilike(f'%{q}%')), User.active = is_active)
        elif not active == "any":
            is_active = True if active == "true" else False
